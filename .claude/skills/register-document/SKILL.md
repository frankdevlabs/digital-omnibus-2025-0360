...
description: >-
  Register ONE new official document on file 2025/0360 (COD) into the repo's
  single source of truth. Use when asked to add / register / file a new document
...
1. **Gather metadata.** From the URL (WebFetch) and/or the local file, establish:
   ...
   - For Dutch-language sources, apply the scope rule: in-scope only if it concerns the data/cyber
     Omnibus **2025/0360**, not the AI-Act file 2025/0359.
...
2. **Pick the source folder by `body`** and commit the file there with the repo's filename
   ...
   | Parliament | `sources/parliament/` | `JURI-PA-789142_draft-opinion_2026-06-05.pdf` (committee operative text — commit it; metadata-only/link if text not retrievable) |
...
5. **Route the substance** to the right layer:
   ...
   - **Member-state / national-government position** → add a section to
     ...
     from the `WK` delegation stream) and cross-link the relevant `docs/provisions/*.md`. A
     national-government **position letter** stating a Member State's own stance — a Dutch *Kamerbrief*,
     a **BNC-fiche**, a **non-paper** — is itself a primary source: commit it here (e.g.
     `NL-KAMERBRIEF-2026-06-18`, `NL-NONPAPER-2026-04-08`), do not merely cite it by URL. Procedural
     reportage (debate reports, another institution's reaction/cover letter) stays analysis-layer.
...
6. **If the document carries operative text, hand transcription off — don't transcribe here:**
   - A full **Council compromise text** (new ST LIMITE text) → **`transcribe-council-extract`**
     (`extracts/council/` five-file set).
   - An **EP committee text** (JURI/LIBE/ITRE/IMCO draft opinion, draft report, or tabled amendments)
     ...
     register metadata-only and set `pending_operative_text: true` (see `resolve-tracker-issue`) —
     do not assert operative content from a screenshot or summary.
