...
2. **Gather evidence.** Fetch `source_url` (WebFetch) and compare against what the repo
   ...
   - **doceo HTTP-202 / WAF workaround.** `www.europarl.europa.eu/doceo/...` (EP committee PDFs
     ...
     committee *documents* listing pages and OEIL, but you **cannot fetch its operative text**
     server-side. Get it via a browser download or a non-WAF mirror. If only metadata/a cover
