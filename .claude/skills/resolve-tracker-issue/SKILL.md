...
description: >-
  Triage and integrate ONE flagged tracker finding for file 2025/0360 (COD).
...
1. **Resolve the issue.** Run the helper to get the parsed, watchlist-joined record:
   ...
   set of downstream files to consider — it comes from `data/watchlist.yaml`.
...
2. **Gather evidence.** Fetch `source_url` (WebFetch) and compare against what the repo
   ...
   live source and, for Dutch-language sources (T1-10/T1-11/T1-14/T1-16/T2-07), apply the
   scope rule: in-scope only if it concerns the data/cyber Omnibus **2025/0360**, not the
   AI-Act file 2025/0359. If it is the AI-Act file or otherwise off-topic, the plan's
   recommendation is to **close the issue as not-relevant** — do not edit the repo.
   - **Scope tie-breaker for EP committee docs.** A committee may opine on *both* omnibus
     files; a PE-number can be mis-attributed by a listing-page summary. Confirm the
     **interinstitutional reference on the document itself** (`2025/0360(COD)`, data/cyber —
     which **includes the Data Act strand** of COM(2025) 837 — vs `2025/0359(COD)`, AI) and
     the **rapporteur** (e.g. JURI on 0360 = Benifei; JURI on 0359 = Lagodinsky). Cross-check
     OEIL for *both* files before deciding scope.
   - **Tier-3 can carry Tier-1 substance — upgrade the playbook.** The flagged source's tier
     sets priority, not the *type* of integration. If a Tier-2/Tier-3 item (NGO/media/Bluesky)
     surfaces a **primary institutional document** (a Council text, an EP committee opinion/
     report/amendments, an advisory opinion), do **not** stay on `stakeholder-social` — run the
     matching `institutional` / `council-text` / `parliament-text` playbook on the underlying
     document. (This is the #47 pattern: a `T3-01` Bluesky post surfaced a JURI draft opinion.)
   - **doceo HTTP-202 / WAF workaround.** `www.europarl.europa.eu/doceo/...` (EP committee PDFs
     and DOCX) returns HTTP 202 with an AWS-WAF JS challenge to all non-browser clients —
     `curl`/WebFetch get 0 bytes. You can confirm a document's **existence + metadata** via the
     committee *documents* listing pages and OEIL, but a plain `curl`/WebFetch **cannot fetch its operative text** server-side. **First run the shared fetcher** (it drives a headless browser and solves the WAF): `python3 $HOME/law-tracker/lib/fetch_blocked_doc.py "<doceo-url>" "sources/parliament/<DOC-ID>_<slug>_<date>.pdf"` — exit 0 → the operative text is in hand, hand off to `transcribe-parliament-extract`; exit 2/4 → genuinely unretrievable or the fetcher is not set up (`~/law-tracker/lib/SETUP.md`), then fall back to a manual browser download or a non-WAF mirror. If only metadata/a cover
     page is confirmed, register **metadata only** and set `pending_operative_text` (below) — do
     not assert per-provision content from a screenshot or a journalist's summary.
...
A committee text (JURI/LIBE/ITRE/IMCO draft opinion, joint ITRE/LIBE draft report, or tabled
amendments) is the Parliament analogue of `council-text`. Often it arrives indirectly (via a Tier-2/3
source — see the upgrade rule above) because the watchlist's EP coverage is thin.
- **Confirm scope first** (the tie-breaker above): file `2025/0360`, correct rapporteur.
...
- **If the operative text is in hand** (browser download / mirror committed under
  ...
  column format — **not** the council five-file consolidated split). Then cascade like `council-text`:
  the affected `docs/provisions/*.md` Parliament sections **or** `docs/instruments/*.md` (e.g. the Data
  Act strand → [`docs/instruments/data-act-2023-2854.md`](../../../docs/instruments/data-act-2023-2854.md)),
  the `parliament` cells in `data/positions.csv`, `docs/institutional-positions.md`, and
  `STATUS.md`/`docs/what-changed.md` if a tracked outcome moved.
...
reportage — register it in `data/documents.yaml` and commit it under `sources/member-states/` (like
`NL-NONPAPER-2026-04-08`), then route the substance to `docs/member-state-positions.md`. These are
almost always fetchable (officielebekendmakingen.nl / tweedekamer.nl downloads, no WAF). Distinguish
from **procedural reportage** — parliamentary *debate reports*, another institution's *reaction/cover
letter*, meeting minutes — which stays analysis-layer (TIMELINE + positions, cited by URL). Worked
example of the trap: triage #57 first declined to commit the 18 Jun NL Kamerbrief *"Naderende
Raadspositie"* by over-analogising it to the #51 Commission reaction letter; it should have been — and
now is — a committed `sources/member-states/` source.
...
- Check its `url` in `data/watchlist.yaml` (and `data/documents.yaml`). Is it stale,
...
- Propose: a corrected URL, a fallback endpoint, or marking the source unreachable.
  Defunct feed (e.g. T2-10) → propose removal/replacement in `watchlist.yaml` and the cron
  prompt. This mirrors the earlier source-health PR pattern. The plan lists the per-source
  verdict; `--fix` edits `watchlist.yaml`/`documents.yaml` accordingly.
...
- Add one **Next milestones to watch** line in `STATUS.md` naming the document and the retrieval
  condition (e.g. "JURI PE789.142 operative text — pending doceo/mirror").
